...
<style>
    ...
    .main-content {
        display: flex;
        flex-direction: row; /* Keep as row for side-by-side display */
...
.sidebar-menu li a img {
    ...
    .table tr:hover {
    background-color: #ffffff !important; /* Use !important to enforce this rule */
    }
        ...
        <div class="table-container">
            ...
            <table class="table">
                ...
                <tbody>
                    ...
                    @php
                    // Assuming you have access to the transaction relationship
                    $latestTransaction = $vehicle->transactions()->latest()->first(); // Get the latest transaction
                    $stickerExpiry = $latestTransaction ? $latestTransaction->sticker_expiry : null; // Get sticker expiry if exists
                    $isNearExpiration = $stickerExpiry && now()->diffInDays($stickerExpiry) <= 30; // Check if near expiration
                @endphp
                        <tr>
